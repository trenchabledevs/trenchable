...
function extractMint(): string | null {
  const match = window.location.pathname.match(PLATFORM_PATTERNS.pumpfun.pathRegex);
...
async function scanAndInject(mint: string) {
  console.log(`[Trenchable] pump.fun: Scanning ${mint}...`);
...
// Also use Navigation API where available
try {
